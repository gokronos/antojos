...
function ProductModal({ product, categories, saving, onChange, onClose, onSave }: { product: Product; categories:Category[]; saving: boolean; onChange: (product: Product) => void; onClose: () => void; onSave: () => void }) {
  return <div className="modal-back"><form className="edit-modal" onSubmit={(event) => { event.preventDefault(); onSave(); }}>
    ...
    <div className="form-row"><label>Precio del producto<input required min="0" type="number" value={product.price} onChange={(event) => onChange({ ...product, price: Number(event.target.value) })} /></label><label>Precio del recipiente<input required min="0" step="100" type="number" value={product.packagingFee} onChange={(event) => onChange({ ...product, packagingFee: Number(event.target.value) })} /><small>Use 0 si no necesita empaque</small></label></div>
    <label>Categoría<select required value={product.category} onChange={(event) => onChange({ ...product, category: event.target.value })}>{categories.map((category)=><option key={category.id}>{category.name}</option>)}</select></label>
