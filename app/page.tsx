...
export default function Home() {
  ...
  const packagingTotal = (isTakeaway||isDelivery) ? cart.reduce((sum,item)=>sum+item.packagingFee*item.qty,0) : 0;
  ...
  async function submitOrder() {
    if (!isOpenNow) {
      setError("El local está cerrado según el horario de atención.");
      setCheckout(false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          customerPhone,address,neighborhood,addressReference,paymentMethod,saveCustomer,
        }),
      });
      const result = await response.json();
  ...
  async function submitServiceRequest() {
    if(!isOpenNow){setError("El local está cerrado según el horario de atención.");setServiceOpen(false);return;}
